...
- Add-on catalog: upstream-first [manifest/addons.json](manifest/addons.json) (curated / git clone targets), [scripts/generate_addon_catalog.py](scripts/generate_addon_catalog.py) (validate + baked C), optional [scripts/emit_curated_addon_manifest.py](scripts/emit_curated_addon_manifest.py) to rebuild the slim list from a prior export, and [docs/synastria-monorepo-addons.md](docs/synastria-monorepo-addons.md) for the **separate** `synastria-monorepo-addons` git repo (sibling directory; publish with its own remote). Felbite bulk import: [scripts/import_web_catalogs.py](scripts/import_web_catalogs.py) with `--sources felbite` only.

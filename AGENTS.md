...
- **Add-on catalog (canonical):** [synastria-monorepo-addons `manifest/addons.json`](https://github.com/RosemyneH/synastria-monorepo-addons) — clone **next to** this repo or as `synastria-monorepo-addons/`. Addons are **hub + GitHub (or vendored paths in the hub)** only; see [docs/synastria-monorepo-addons.md](docs/synastria-monorepo-addons.md). [scripts/generate_addon_catalog.py](scripts/generate_addon_catalog.py) (validate + baked C): `python scripts/generate_addon_catalog.py --check` resolves the hub like CMake (`--input` or `AHC_HUB_ADDONS_JSON` if needed). [build-app.bat](build-app.bat) / [build-test.bat](build-test.bat) run [scripts/ensure-synastria-hub.bat](scripts/ensure-synastria-hub.bat), which **clones if missing** but does not `git pull` an existing clone. To check `origin/main` and print `Validated N addons`, run [scripts/verify-synastria-hub-freshness.bat](scripts/verify-synastria-hub-freshness.bat) with the repo root. [docs/MANUAL_TEST_CATALOG.md](docs/MANUAL_TEST_CATALOG.md). Optional: [emit_curated_addon_manifest.py](scripts/emit_curated_addon_manifest.py). SynScoots list helper: [scripts/list_synastoots_repos.py](scripts/list_synastoots_repos.py).

...
- **Project rule (always on)**: [.cursor/rules/verify-build-and-test.mdc](.cursor/rules/verify-build-and-test.mdc) — when changing C, CMake, manifest, or the addon catalog script, run the build and tests (or the catalog check) as described there.

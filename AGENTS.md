...
- **Add-on catalog (canonical):** [synastria-monorepo-addons `manifest/addons.json`](https://github.com/RosemyneH/synastria-monorepo-addons) — clone **next to** this repo or as `synastria-monorepo-addons/`. [scripts/generate_addon_catalog.py](scripts/generate_addon_catalog.py) (validate + baked C) with `--input ../synastria-monorepo-addons/manifest/addons.json`. [docs/synastria-monorepo-addons.md](docs/synastria-monorepo-addons.md) · [docs/MANUAL_TEST_CATALOG.md](docs/MANUAL_TEST_CATALOG.md). Optional: [emit_curated_addon_manifest.py](scripts/emit_curated_addon_manifest.py). SynScoots list helper: [scripts/list_synastoots_repos.py](scripts/list_synastoots_repos.py). Felbite bulk import (maintenance): [import_web_catalogs.py](scripts/import_web_catalogs.py) with `--sources felbite` only.

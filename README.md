...
**Automated GitHub Release:** on a **tag** whose name starts with `v` (e.g. `v0.1.0`), an extra job publishes a **Release** and attaches the Windows `.exe` and the Linux binary using [softprops/action-gh-release](https://github.com/softprops/action-gh-release), with [generated release notes](https://docs.github.com/en/repositories/releasing-projects-on-github/automatically-generated-release-notes). The repository must allow the default `GITHUB_TOKEN` to **write** contents (Settings → Actions → General → Workflow permissions, or the org default). Example:
